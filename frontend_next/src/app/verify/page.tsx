...
export default function CertificateVerification() {
  ...
  const [certificateDetails, setCertificateDetails] = useState<CertificateDetails | null>(null); // Holds certificate details after verification
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      if (data.success) {
        setCertificateDetails(data.cert); // Set certificate details on successful verification
      } else {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-black text-white bg-[linear-gradient(to_bottom,#000,#0A1A33_34%,#113366_65%,#335B99_82%)] py-4 sm:py-8 relative overflow-hidden">
      ...
      {certificateDetails && (
        <div className="mt-6 bg-white bg-opacity-10 backdrop-blur-md p-8 rounded-lg shadow-lg w-full max-w-md">
          ...
          <div className="text-sm mb-1"><strong>Event Name:</strong> {certificateDetails.eventName}</div> {/* Display event name */}
          <div className="text-sm mb-1"><strong>Organized By:</strong> {certificateDetails.organizedBy}</div> {/* Display organizer */}
          <div className="text-sm mb-1"><strong>Prize:</strong> {certificateDetails.prize}</div> {/* Display prize */}
          <div className="text-sm mb-1"><strong>Date & Time:</strong> {certificateDetails.dateTime}</div> {/* Display date and time */}
          <div className="text-sm mb-1"><strong>Issued To:</strong> {certificateDetails.issuedTo}</div> {/* Display recipient name */}
          <div className="text-sm"><strong>Issued to Email Id:</strong> {certificateDetails.issuedToEmail}</div> {/* Display recipient email */}
        </div>
      )}
    </div>
  );
